/** kitadmin-v2.1.0 MIT License By http://kit.zhengjinfan.cn Author Van Zheng */
;
"use strict";
var mods = ["element", "sidebar", "mockjs", "select", "tabs", "menu", "route", "utils", "component", "kit"];
layui.define(mods, function(e) {
	layui.element;
	var t = layui.utils,
		a = layui.jquery,
		n = (layui.lodash, layui.route),
		i = layui.tabs,
		l = layui.layer,
		o = layui.menu,
		m = layui.component,
		s = layui.kit,
		p = function() {
			this.config = {
				elem: "#app",
				loadType: "SPA"
			}, this.version = "1.0.0"
		};
	p.prototype.ready = function(e) {
		var i = this.config,
			o = (0, t.localStorage.getItem)("KITADMIN_SETTING_LOADTYPE");
		null !== o && void 0 !== o.loadType && (i.loadType = o.loadType), s.set({
			type: i.loadType
		}).init(),

			//初始化tab选项卡页面为
			u.routeInit(i), u.menuInit(i), "TABS" === i.loadType && u.tabsInit(), "" === location.hash && t.setUrlState("主页", "#/"),

			a("#cc").on("click", function() {
			layui.sidebar.render({
				elem: this,
				title: "项目信息",
				shade: !0,
				dynamicRender: !0,
				//javascript:;直接调用后台controller对应mapping
				url: "/projectInfo",
				width: "50%"
			})
        }), 
        m.on("nav(header_right)", function(e) {
			var t = e.elem.attr("kit-target");
			"setting" === t && layui.sidebar.render({
				elem: e.elem,
				title: "设置",
				shade: !0,
				dynamicRender: !0,
				//javascript:;直接调用后台controller对应mapping
				url: "/setting"
            })

        }),
        layui.mockjs.inject(APIs), "SPA" === i.loadType && n.render(), "function" == typeof e && e()
	};
	var u = {
		routeInit: function(e) {
			var t = this,
				a = {

					routes: [

						{
						//	当url为"/"时,GetMapping到controller的@RequestMapping("/")方法,获取main页面
						path: "/login",
						component: "/login",
						name: "登录"

					},

						{
						//	当url为"/"时,GetMapping到controller的@RequestMapping("/")方法,获取main页面
						path: "/",
						component: "/main",
						name: "主页"
					},

						{
						path: "/user/info",
						component: "/user/info",
						name: "用户管理"
					},

						{
						path: "/role/info",
						component: "/role/info",
						name: "角色管理"
					},

						{
						path: "/permission/info",
						component: "/permission/info",
						name: "权限管理"
					},

						{
						path: "/exception/403",
						component: "views/exception/403.html",
						name: "403"
					},

						{
						path: "/exception/404",
						component: "views/exception/404.html",
						name: "404"
					},

						{
						path: "/exception/500",
						component: "views/exception/500.html",
						name: "500"
					}]
				};
			return "TABS" === e.loadType && (a.onChanged = function() {
				i.existsByPath(location.hash) ? i.switchByPath(location.hash) : t.addTab(location.hash, (new Date).getTime())
			}), n.setRoutes(a), this
		},
		addTab: function(e, t) {
			var a = n.getRoute(e);
			a && i.add({
				id: t,
				title: a.name,
				path: e,
				component: a.component,
				rendered: !1,
				icon: "&#xe62e;"
			})
		},

		menuInit: function(e) {
			var t = this;
			return o.set({
				dynamicRender: !1,
				isJump: "SPA" === e.loadType,
				onClicked: function(a) {
					if ("TABS" === e.loadType && !a.hasChild) {
						var n = a.data,
							i = n.href,
							l = n.layid;
						t.addTab(i, l)
					}
				},
				elem: "#menu-box",
				remote: {
					url: "/api/getmenus",
					method: "post"
				},
				cached: !1
			}).render(), t
		},

		//选项卡模式初始化页面
		tabsInit: function() {
			i.set({
				onChanged: function(e) {}
			}).render(function(e) {
				e.isIndex && n.render("#/")
			})
		}

	};

	(new p).ready(function() {
		console.log("Init successed.")
	}), e("admin", {})

});
